...
class BayesBiNN(Optimizer):
    ...
    def __init__(self, parameters, train_set_size, lr=1e-9, betas=0.0, prior_lamda=None, num_samples=5,lamda_init = 10, lamda_std = 0, temperature=1,reweight=1):
        if lr <= 0.0:
        ...
        self.train_modules = []
        self.set_train_modules(parameters)  # to obtain the trained modules in the model
    ...
    def set_train_modules(self, parameters):
        for child in list(parameters):
            self.train_modules.append(child)
